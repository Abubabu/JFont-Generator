...
public class Overlay4 {
private final getImage testCase;
	
	public Overlay4(String filepath) throws IOException
	{
	...
	/*	int aspectWidthA = asFractiondenomanator(aHeight,aWidth);
		...
		for(int ay = TestCenterY - TestSquareSideLength/2, by = UserCenterY - UserSquareSideLength/2; ay < (TestCenterY + TestSquareSideLength/2) / aH;  ay += aH , by += bH)//y  apix[ycoord][xcoord]
		{
			for(int ax = TestCenterX - TestSquareSideLength/2, bx = UserCenterX - UserSquareSideLength/2; ay < (TestCenterX + TestSquareSideLength/2) / aW;  ax += aW, bx += bW)//x
			{
